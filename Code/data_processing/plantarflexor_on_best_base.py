...
def measure(policy, base_strength, pf_strength_factor, base_vmax=1.0, max_steps=1000, contact_threshold=10.0):
    ...
    scaled[:, 6] = original_gainprm[:, 6] * base_vmax
...
if __name__ == "__main__":
    ...
    print("Testing plantarflexor targeting on strength=0.6 base (our widest stable range)\n")
    pf_factors = [1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3]
    print("pf_factor, steps, conf, ds_pct, n_strikes")
    for pf in pf_factors:
        steps, conf, ds, n = measure(policy, base_strength=0.6, pf_strength_factor=pf)
