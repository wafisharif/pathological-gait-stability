...
def run_and_measure(policy, strength_factor, dynamics_slowdown, max_steps=1000):
    """
    dynamics_slowdown: multiplier on BOTH tau_act and tau_deact (preserves
    their ratio). 1.0 = normal speed. Higher = muscles activate/deactivate
    more slowly -- a distinct mechanism from strength (force capacity) and
    from action magnitude (policy's commanded signal, unchanged here).
    """
    env = gym.make('myoLegWalk-v0')
    sim = env.unwrapped.sim

    original_gainprm = sim.model.actuator_gainprm.copy()
    scaled_gain = original_gainprm.copy()
    scaled_gain[:, 2] = original_gainprm[:, 2] * strength_factor
    sim.model.actuator_gainprm[:] = scaled_gain

    original_dynprm = sim.model.actuator_dynprm.copy()
    scaled_dyn = original_dynprm.copy()
    scaled_dyn[:, 0] = original_dynprm[:, 0] * dynamics_slowdown
    scaled_dyn[:, 1] = original_dynprm[:, 1] * dynamics_slowdown
    sim.model.actuator_dynprm[:] = scaled_dyn
    ...
    if len(feet_heights_post) < 5:
        return {"strength_factor": strength_factor, "dynamics_slowdown": dynamics_slowdown,
...
if __name__ == "__main__":
    ...
    slowdowns = [1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 7.0, 10.0]
    results_path = "Results/als_strength_plus_dynamics.csv"
    header = ["strength_factor", "dynamics_slowdown", "steps_survived", "distance", "measured_speed",
    ...
    print("slowdown, steps, dist, speed, conf, cadence, stride_len, dbl_supp%, single_supp%, n_strikes")
    for sd in slowdowns:
        r = run_and_measure(policy, strength_factor=FIXED_STRENGTH, dynamics_slowdown=sd)
        print(f"{r['dynamics_slowdown']}, {r['steps_survived']}, {r['distance']:.2f}, {r['measured_speed']:.4f}, "
