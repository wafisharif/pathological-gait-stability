...
# Distal/ankle-foot muscle group, both legs -- the literature-supported
# ALS-preferential-weakness target (foot drop pattern)
DISTAL_MUSCLE_NAMES = [
    'soleus', 'gaslat', 'gasmed', 'tibant', 'tibpost',
    'perlong', 'perbrev', 'edl', 'fdl', 'ehl', 'fhl'
]
...
def run_and_measure(policy, distal_strength_factor, max_steps=1000):
    env = gym.make('myoLegWalk-v0')
    sim = env.unwrapped.sim

    muscle_names = [sim.model.actuator(i).name for i in range(sim.model.nu)]
    distal_indices = [i for i, name in enumerate(muscle_names)
                        if any(name.startswith(d) for d in DISTAL_MUSCLE_NAMES)]

    original_gainprm = sim.model.actuator_gainprm.copy()
    scaled = original_gainprm.copy()
    scaled[distal_indices, 2] = original_gainprm[distal_indices, 2] * distal_strength_factor
    ...
    if len(feet_heights_post) < 5:
        return {"distal_strength": distal_strength_factor, "steps_survived": steps_survived,
                "distance": distance, "measured_speed": measured_speed, "confidence": "too_short",
    ...
    return {"distal_strength": distal_strength_factor, "steps_survived": steps_survived,
            "distance": distance, "measured_speed": measured_speed, "confidence": confidence,
...
if __name__ == "__main__":
    ...
    distal_indices = [i for i, name in enumerate(muscle_names) if any(name.startswith(d) for d in DISTAL_MUSCLE_NAMES)]
    print(f"Distal muscles found: {len(distal_indices)} -- {[muscle_names[i] for i in distal_indices]}")
    env.close()

    distal_levels = [1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2]
    results_path = "Results/als_distal_strength.csv"
    header = ["distal_strength", "steps_survived", "distance", "measured_speed", "confidence",
               "cadence_per_1000steps", "stride_length_approx", "double_support_pct",
               "single_support_pct", "n_total_strikes"]

    print("distal_strength, steps, dist, speed, conf, cadence, stride_len, dbl_supp%, single_supp%, n_strikes")
    for d in distal_levels:
        r = run_and_measure(policy, distal_strength_factor=d)
        print(f"{r['distal_strength']}, {r['steps_survived']}, {r['distance']:.2f}, {r['measured_speed']:.4f}, "
