...
# Confirmed sensor mapping: r_foot+r_toes = right, l_foot+l_toes = left
...
def measure_stroke(policy, paretic_side, paretic_strength, max_steps=1000):
    ...
    # paretic = the weakened side; non-paretic = full strength side
    if paretic_side == "left":
    ...
    return {
        "paretic_strength": paretic_strength,
        "steps": steps,
        "n_strikes": int(n_strikes),
        "distance": round(distance, 2),
        "paretic_stance_pct": round(paretic_stance, 2),
...
if __name__ == "__main__":
    ...
    print("STROKE ASYMMETRY SWEEP — weakening LEFT leg progressively\n")
    ...
    for ps in [1.0, 0.8, 0.6, 0.5, 0.4, 0.3, 0.2]:
        r = measure_stroke(policy, paretic_side="left", paretic_strength=ps)
        if r is None:
